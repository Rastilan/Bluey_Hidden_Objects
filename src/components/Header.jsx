...
import { selectUserName, selectUserEmail, selectUserPhoto, setUserLoginDetails, setSignOutState, setUserDocSnapshot } from '../features/user/userSlice.jsx';
...
const Header = (props) => {
    ...
    const createFirebaseData = async (userDocRef) => {
    ...
    useEffect(() => {
        const fetchUserSnapshotData = async () => {
          try {
            // Check if userName is not empty or undefined and user is authenticated
            if (userName && auth.currentUser) {
              const userDocRef = doc(db, "userdata", userName);
              const userDocSnapshot = await getDoc(userDocRef);
              dispatch(setUserDocSnapshot(userDocSnapshot));
      
              // Save LongDog status for new users
